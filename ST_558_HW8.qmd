...
```{r}
# Filter data for functioning days and summarize statistics
functioning_data <- df

# Summary of bike count across categories
...
```{r}
# Filter data for functioning days and summarize statistics
# Filter data for functioning days
functioning_data <- df %>%
...
```{r}
# Verify that 'date' is of class Date
class(df_daily$date)
```
...
recipe1 <- recipe(rented_bike_count ~ ., data = train_data) %>%
  ...
  # Remove original date variables
...
# Bake the recipe to check the data
...
recipe2 <- recipe(rented_bike_count ~ ., data = train_data) %>%
  ...
  # Remove original date variables
  step_rm(date, date_dow) %>%
  # Create interactions before dummy variables
